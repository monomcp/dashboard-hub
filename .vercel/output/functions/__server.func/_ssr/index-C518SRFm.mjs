...
import { M as Menu, r as CircleCheck, l as Search, d as CircleQuestionMark, P as Plus, ax as UserRound, ai as Bot, au as SquareCheckBig, ay as TriangleAlert, az as CalendarClock, x as Star, C as ChevronUp, E as EllipsisVertical, b as Check, aA as Circle, aB as CircleCheckBig, L as LoaderCircle, ac as Clock, aC as TextAlignStart, aD as ListChecks, as as CalendarDays, at as ChevronLeft, Y as ChevronRight, O as ChevronDown } from "../_libs/lucide-react.mjs";
...
const openStatuses = /* @__PURE__ */ new Set(["todo", "claimed", "in_progress", "needs_review", "failed", "blocked", "open"]);
const taskAssignments = (task) => task.assignments ?? [];
function TasksPage() {
  const navigate = useNavigate();
  const [lists, setLists] = reactExports.useState([]);
  const [tasksByList, setTasksByList] = reactExports.useState({});
  const [principals, setPrincipals] = reactExports.useState([]);
  ...
  const [taskStatus, setTaskStatus] = reactExports.useState("todo");
  const [taskParentId, setTaskParentId] = reactExports.useState("none");
  const [taskPrincipalId, setTaskPrincipalId] = reactExports.useState("none");
  const [taskAssignmentRole, setTaskAssignmentRole] = reactExports.useState("assignee");
  ...
  const loadTasks = reactExports.useCallback(async () => {
    ...
    try {
      ...
      const principalPage = await apiRequest("/api/v1/principals?limit=200");
  ...
  const visibleLists = reactExports.useMemo(() => {
    if (["all", "my", "agents", "needs_review", "failed", "scheduled", "starred"].includes(view)) return lists;
    return lists.filter((l) => l.id === view);
  }, [lists, view]);
  const principalById = reactExports.useMemo(() => new Map(principals.map((principal) => [principal.id, principal])), [principals]);
  const myPrincipal = reactExports.useMemo(() => principals.find((principal) => principal.type === "user") ?? null, [principals]);
  const allTasks = reactExports.useMemo(() => Object.values(tasksByList).flat(), [tasksByList]);
  const editingTask = reactExports.useMemo(() => allTasks.find((task) => task.id === editingTaskId) ?? null, [allTasks, editingTaskId]);
  ...
  const sortedTasks = reactExports.useCallback((list) => {
    ...
    const filtered = tasks.filter((task) => {
      ...
      if (view === "my") {
        return !!myPrincipal && taskAssignments(task).some((a) => a.principal_id === myPrincipal.id);
      }
      if (view === "agents") {
        return taskAssignments(task).some((a) => principalById.get(a.principal_id)?.type === "agent");
      }
  ...
  const openNewTask = (listId, parentTaskId = null) => {
    if (!listId) return;
  ...
  const openNewSubtask = (parentTask) => {
  ...
  const saveTask = async () => {
    ...
    try {
      const payload = {
        ...
        assignments: taskPrincipalId === "none" ? [] : [{
          principal_id: taskPrincipalId,
  ...
  const toggleDone = async (task) => {
    const nextStatus = isDone(task) ? "todo" : "done";
    setTasksByList((prev) => ({
      ...prev,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex", children: [
      sidebarOpen && /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "hidden w-[260px] shrink-0 px-3 md:block", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "space-y-1", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(SidebarItem, { icon: /* @__PURE__ */ jsxRuntimeExports.jsx(UserRound, { className: "h-5 w-5" }), label: "My tasks", active: view === "my", onClick: () => setView("my") }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(SidebarItem, { icon: /* @__PURE__ */ jsxRuntimeExports.jsx(Bot, { className: "h-5 w-5" }), label: "Assigned to agents", active: view === "agents", onClick: () => setView("agents") }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(SidebarItem, { icon: /* @__PURE__ */ jsxRuntimeExports.jsx(SquareCheckBig, { className: "h-5 w-5" }), label: "Needs review", active: view === "needs_review", onClick: () => setView("needs_review") }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(SidebarItem, { icon: /* @__PURE__ */ jsxRuntimeExports.jsx(TriangleAlert, { className: "h-5 w-5" }), label: "Failed", active: view === "failed", onClick: () => setView("failed") }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(SidebarItem, { icon: /* @__PURE__ */ jsxRuntimeExports.jsx(CalendarClock, { className: "h-5 w-5" }), label: "Scheduled", active: view === "scheduled", onClick: () => setView("scheduled") }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-6", children: [
          ...
          listsExpanded && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-1 space-y-1", children: [
            loading ? /* @__PURE__ */ jsxRuntimeExports.jsx(SidebarListsSkeleton, {}) : lists.map((l) => {
              const open = (tasksByList[l.id] ?? []).filter((t) => openStatuses.has(t.status)).length;
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "flex-1 px-4 pb-16 md:px-6", children: [
        ...
        loading ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-wrap gap-5", children: [
          ...
          visibleLists.map((list) => {
            ...
            const parentTasks = tasks.filter((task) => !task.parent_task_id || !taskIds.has(task.parent_task_id));
            ...
            return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "w-full max-w-[360px] flex-1 basis-[320px] rounded-2xl bg-white p-5 shadow-[0_1px_2px_rgba(16,24,40,0.04),0_1px_3px_rgba(16,24,40,0.06)]", children: [
              ...
              showEmpty ? /* @__PURE__ */ jsxRuntimeExports.jsx(EmptyState, {}) : /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: "space-y-1", children: parentTasks.map((t) => {
                const done = isDone(t);
                const assignment = taskAssignments(t)[0];
                const assignee = assignment ? principalById.get(assignment.principal_id) : null;
                const subtasks = tasks.filter((task) => task.parent_task_id === t.id);
                const childCount = subtasks.length;
                return /* @__PURE__ */ jsxRuntimeExports.jsxs("li", { className: "group rounded-lg px-1 py-2 hover:bg-stone-50", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => openEditTask(t), className: "flex-1 text-left", children: [
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground", children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "rounded-full bg-stone-100 px-2 py-0.5", children: t.status.replace("_", " ") }),
                        assignee && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "rounded-full bg-sky-50 px-2 py-0.5 text-sky-700", children: [
                          assignment.role,
                          ": ",
                          assignee.name
                        ] }),
                        t.claimed_by_principal_id && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "rounded-full bg-amber-50 px-2 py-0.5 text-amber-700", children: "claimed" }),
                        childCount > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "rounded-full bg-emerald-50 px-2 py-0.5 text-emerald-700", children: [
                          childCount,
                          " subtasks"
                        ] })
                      ] })
                  ...
                  subtasks.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: "mt-2 space-y-1 border-l border-stone-200 pl-5", children: subtasks.map((subtask) => {
                    const subtaskDone = isDone(subtask);
                    return /* @__PURE__ */ jsxRuntimeExports.jsxs("li", { className: "flex items-center gap-2", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => toggleDone(subtask), className: "shrink-0", "aria-label": "Toggle subtask complete", children: subtaskDone ? /* @__PURE__ */ jsxRuntimeExports.jsx(CircleCheckBig, { className: "h-4 w-4 text-sky-600" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Circle, { className: "h-4 w-4 text-foreground/40 transition hover:text-foreground" }) }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => openEditTask(subtask), className: cn("min-w-0 flex-1 truncate text-left text-xs", subtaskDone && "text-muted-foreground line-through"), children: subtask.title })
                    ] }, subtask.id);
                  }) })
                ] }, t.id);
              }) })
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: taskDialogOpen, onOpenChange: setTaskDialogOpen, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogContent, { className: "rounded-2xl sm:max-w-xl", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4 pt-2", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid gap-3 sm:grid-cols-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: taskStatus, onValueChange: (v) => setTaskStatus(v), children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(SelectTrigger, { className: "bg-stone-100 border-0 rounded-lg", children: /* @__PURE__ */ jsxRuntimeExports.jsx(SelectValue, { placeholder: "Status" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(SelectContent, { children: ["todo", "claimed", "in_progress", "needs_review", "failed", "blocked", "done"].map((status) => /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: status, children: status.replace("_", " ") }, status)) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: taskParentId, onValueChange: setTaskParentId, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(SelectTrigger, { className: "bg-stone-100 border-0 rounded-lg", children: /* @__PURE__ */ jsxRuntimeExports.jsx(SelectValue, { placeholder: "Parent task" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(SelectContent, { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: "none", children: "No parent task" }),
              allTasks.filter((task) => task.id !== editingTaskId && task.parent_task_id === null).map((task) => /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: task.id, children: task.title }, task.id))
            ] })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid gap-3 sm:grid-cols-[1fr_150px]", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: taskPrincipalId, onValueChange: setTaskPrincipalId, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(SelectTrigger, { className: "bg-stone-100 border-0 rounded-lg", children: /* @__PURE__ */ jsxRuntimeExports.jsx(SelectValue, { placeholder: "Assign principal" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(SelectContent, { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: "none", children: "Unassigned" }),
              principals.map((principal) => /* @__PURE__ */ jsxRuntimeExports.jsxs(SelectItem, { value: principal.id, children: [
                principal.name,
                " · ",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: taskAssignmentRole, onValueChange: (v) => setTaskAssignmentRole(v), children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(SelectTrigger, { className: "bg-stone-100 border-0 rounded-lg", children: /* @__PURE__ */ jsxRuntimeExports.jsx(SelectValue, {}) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(SelectContent, { children: ["assignee", "reviewer", "approver", "watcher"].map((role) => /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: role, children: role }, role)) })
          ] })
        ] }),
        editingTaskId && !editingTaskIsSubtask && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2 rounded-xl border border-stone-200 p-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between gap-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm font-medium", children: "Subtasks" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(Button, { type: "button", variant: "ghost", size: "sm", className: "h-8 rounded-full text-sky-600", onClick: () => {
          ...
          allTasks.filter((task) => task.parent_task_id === editingTaskId).length > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: "space-y-1", children: allTasks.filter((task) => task.parent_task_id === editingTaskId).map((subtask) => /* @__PURE__ */ jsxRuntimeExports.jsxs("li", { className: "flex items-center gap-2 rounded-lg bg-stone-50 px-3 py-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", onClick: () => toggleDone(subtask), "aria-label": "Toggle subtask complete", children: isDone(subtask) ? /* @__PURE__ */ jsxRuntimeExports.jsx(CircleCheckBig, { className: "h-4 w-4 text-sky-600" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Circle, { className: "h-4 w-4 text-foreground/40" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", onClick: () => openEditTask(subtask), className: cn("min-w-0 flex-1 truncate text-left text-sm", isDone(subtask) && "text-muted-foreground line-through"), children: subtask.title }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "rounded-full bg-white px-2 py-0.5 text-[11px] text-muted-foreground", children: subtask.status.replace("_", " ") })
          ] }, subtask.id)) }) : /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground", children: "No subtasks yet." })
        ] })
      ] }),

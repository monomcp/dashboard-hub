...
// Shared activity log.
//
// Renders the org's request logs (Console UI calls + MCP tool calls) from
// /api/v1/request-logs, with filters, infinite scroll and a detail dialog.
// Callers tune it with `logFilter` (e.g. Content scopes to its own tools; the
// Audit log shows everything) and an optional controlled `source`.
// ─────────────────────────────────────────────────────────────────────────────
...
export type ActivityLogProps = {
  ...
  /** Keep only matching logs (e.g. a module's own tools). Defaults to all. */
  logFilter?: (log: RequestLogSummary) => boolean;
  /**
   * Restrict the catalog servers used to resolve user names. Undefined uses the
   * first enabled toolkit across the whole catalog.
   */
...
export function ActivityLog({
  ...
  const [filters, setFilters] = useState<ActivityFilters>(() =>
    controlledSource
      ? { ...EMPTY_ACTIVITY_FILTERS, source: controlledSource }
  ...
  // Keep the source filter in sync when the caller drives it (e.g. audit nav).
  useEffect(() => {
    if (controlledSource === undefined) return;
  ...
  // Best-effort name lookup for user principals, drawn from an enabled toolkit's
  // access matrix. Admin-gated, so a 403/empty result simply leaves principals
  // labelled by short id.
  ...
  // Extra filters beyond the caller-controlled source.
  const filtersActive =
    (controlledSource === undefined && filters.source !== "all") ||
    filters.status !== "all" ||
    ...
    <div className="grid content-start gap-4">
      <div className={cn(card, "flex min-h-28 items-end justify-between gap-4")}>
        <div className="self-center">
          ...
          <p className={cn("mt-2", mutedText)}>{description}</p>
        </div>
        <button
          type="button"
